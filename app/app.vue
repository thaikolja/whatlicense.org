<script setup lang="ts">
  ...
  const icons = ref<IconLink[]>([
    ...
    },
    {
      name: 'Twitter',
      icon: 'mdi:twitter',
      link: config.public?.links?.twitter ?? ''
    },
    {
      name: 'E-Mail',
      icon: 'mdi:envelope',
      link: `mailto:${config.public?.email}`
    }
  ])
  ...
  <div class="min-h-screen flex flex-col bg-pattern selection:bg-tan selection:text-white relative">
    ...
    <header class="w-full max-w-6xl mx-auto px-6 py-8 flex justify-between items-center relative z-20">
      ...
      <div class="flex items-center gap-4 sm:gap-6">
        <div class="hidden md:flex items-center gap-6 mr-4 border-r border-border pr-6">
          <a :href="config.public.links.termsFeed" target="_blank" class="text-[10px] font-bold uppercase tracking-widest text-muted hover:text-charcoal transition-colors">
            About
          </a>
        </div>

        <div class="flex items-center gap-8">
          <NuxtLink v-for="icon in icons" :key="icon.name" :to="icon.link" :title="`whatlicense.org on ${icon.name}`" target="_blank" class="text-muted hover:text-charcoal transition-colors">
            <Icon :id="`icon-${icon.name.toLowerCase()}`" :name="icon.icon" size="16" />
          </NuxtLink>
        </div>
      </div>
    </header>
    ...
    <footer class="py-10 px-6 border-t border-border mt-12 bg-white/30 backdrop-blur-sm">
      <div class="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div class="flex items-center gap-8">
          <NuxtLink v-for="link in footerLinks" :key="link.name" :to="link.link" target="_blank" class="text-muted hover:text-charcoal transition-colors flex items-center gap-2">
            <Icon v-if="link.icon" :id="`icon-${link.name.toLowerCase()}`" :name="link.icon" size="12" />
