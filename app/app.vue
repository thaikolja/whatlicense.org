...
/**
 * App shell: header, NuxtPage main, footer.
 * Pure layout — no client-only APIs so SSG HTML stays valid.
 */
import type { IconLink, FooterLink } from '~/types'
import { useSiteLinks } from '~/composables/useSiteLinks'
import { useSlug } from '~/composables/useSlug'

// resolve PayPal / GitHub / email once for the whole shell
...
// footer nav (legal internal + support external)
...
// freeze year at setup (SSG-safe; avoids client hydration drift)
const copyrightYear = new Date().getFullYear()
</script>

<template>
  <!-- page chrome + dotted cream background -->
  <div class="page-shell bg-pattern">
    <header class="site-header">
      ...
      >
        <!-- stacked brand mark (rotates slightly on hover) -->
        <div class="logo-mark">
      ...
      <div class="container-wide flex flex-col sm:flex-row items-center justify-between gap-5 sm:gap-6">
        ...
        >
          ...
          >
            <Icon
              v-if="link.icon"
              :id="`icon-footer-${useSlug(link.name)}`"
              :name="link.icon"
              size="12"
            />
